...
- **Pick the right model** — Don't use a reasoning model for `git status` questions. See [Lab 2 Ex. 7](labs/02-agent-mode-mastery.md#exercise-7-pick-the-right-model)
- **Keep a `lessons.md`** — One-line entries that stop the agent re-discovering known gotchas. See [Lab 3 Ex. 8](labs/03-prompt-files-and-instructions.md#exercise-8-lessonsmd--stop-re-discovering-the-same-bugs)
- **Package recurring workflows as `SKILL.md`** — Smaller, scoped context beats one giant instruction file. See [Lab 3 Ex. 7](labs/03-prompt-files-and-instructions.md#exercise-7-skills-skillmd--packaging-domain-knowledge)
